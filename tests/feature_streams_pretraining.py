...
def main() -> int:
    ...
    print("[step] Grabing dista_pretraining config...", flush=True)
    ...
    run_step(dash_cli_path, ["set", "wandb.wb_run_name", "PT_FULL"])
    ...
    print("[step] Deactivating node and packet features..." , flush=True)
    run_step(dash_cli_path, ["set", "node_features", "false"])
    run_step(dash_cli_path, ["set", "use_packet_feats", "false"])

    print("[step] Setting run name...", flush=True)
    run_step(dash_cli_path, ["set", "wandb.wb_run_name", "PT_FLOW"])
    ...
    print("[step] Setting run name...", flush=True)
    run_step(dash_cli_path, ["set", "wandb.wb_run_name", "PT_Flow_Packet"])
    ...
    print("[step] Deactivating model saving ...", flush=True)
    run_step(dash_cli_path, ["--profile", "intrusion_detection.save_models", "false"])

    print("[step] Deactivating packet features..." , flush=True)
    run_step(dash_cli_path, ["set", "use_packet_feats", "false"])

    print("[step] Setting run name...", flush=True)
    run_step(dash_cli_path, ["set", "wandb.wb_run_name", "PT_Flow_Node"])
