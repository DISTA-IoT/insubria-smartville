...
def main() -> int:
    ...
    def modalities_sweep():
        run_step(dash_cli_path, ["set", "node_features", "true"])
        run_step(dash_cli_path, ["set", "use_packet_feats", "true"])
        run_step(dash_cli_path, ["set", "wandb.wb_run_name", "full"])
        start_experiment()
        stop_experiment()
        
        run_step(dash_cli_path, ["set", "node_features", "false"])
        run_step(dash_cli_path, ["set", "wandb.wb_run_name", "flow_packet"])
        start_experiment()
        stop_experiment()

        run_step(dash_cli_path, ["set", "use_packet_feats", "false"])
        run_step(dash_cli_path, ["set", "wandb.wb_run_name", "flow"])
        start_experiment()
        stop_experiment()
