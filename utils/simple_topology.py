...
def mountSwitch(templates, curr_switch_label,ip=None,gateway=None):
    ...
    openvswitch=create_node(server, project, 0, -200, template_id, switch1_node_name)
...
def mount_all_hosts(cfg, templates, switch_node_name, curr_node_count=3, fixed_ips=False):
    node_names = []
...
def connect_all(main_switch_node_name,controller_node_name,host_names, fixed_ips=False):
    ...
    create_link(server, project,str(nat_id),0,str(main_switch_id),1)
    ...
    if fixed_ips:
        ...
        create_link(server, project, str(cloud_id),0,str(main_switch_id),2, port_number_1=port_index)
...
def starTopology(cfg, templates):
    ...
    connect_all(main_switch_node_name,controller_node_name,host_names)
    start_all()
