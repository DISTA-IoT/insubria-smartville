...
def get_dhcp_interface_config_file(iface: str, host_name: Optional[str]) -> str:
    if host_name is None:
        return (
            "# autogenerated\n"
            f"# DHCP config for {iface}\n"
            f"auto {iface}\n"
...
def set_dhcp_node_network_interfaces(
        server: Server, 
        project: Project, 
        node_id: str, 
        iface_name: str,
    ...
    get_req = requests.get(f"http://{server.addr}:{server.port}/v2/projects/{project.id}/nodes/{node_id}/files/etc/network/interfaces", auth=(server.user, server.password))
    get_req.raise_for_status()
    existing_content = get_req.text

    # Append the new line to the existing content
    new_line = get_dhcp_interface_config_file(iface_name, host_name)
    updated_content = existing_content + '\n' + new_line

    # Send the updated content back to GNS3
    post_req = requests.post(f"http://{server.addr}:{server.port}/v2/projects/{project.id}/nodes/{node_id}/files/etc/network/interfaces", data=updated_content, auth=(server.user, server.password))
